...
import { Dna, Camera as LucideCamera, RotateCcw } from "lucide-react";
...
export function Camera() {
  ...
  if (photo && foodData) {
    try {
      ...
      const cleanedData =
        typeof foodData === "string"
          ? cleanJsonString(foodData)
      ...
      return (
        <div className="fixed inset-0">
          {/* Frame container */}
          <div className="fixed inset-0  border-[8px] border-white pointer-events-none">
            {/* Top, left and right border (thin transparent) handled by the container border */}
            {/* Bottom thick black section */}
            <div className="absolute bottom-0 left-0 right-0 h-24 bg-gray-100  bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-90 border border-gray-100"></div>
          </div>
